...
def pubs():
    ...
    return render_template("pubs/index.html", pubs = pubs)
...
def show_pub(id):
    ...
    return render_template("pubs/show.html", pub = pub, pubs = pubs)

@pubs_blueprint.route("/pubs/<id>/edit", methods=['GET'])
def edit_pubs(id):
    pub = pub_repository.select(id)
    cities = city_repository.select_all()
    return render_template("pubs/edit.html", pub = pub, cities = cities)
